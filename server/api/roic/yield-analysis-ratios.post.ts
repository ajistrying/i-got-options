...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        const annualResponse = await fetch(
            `https://api.roic.ai/v2/fundamental/ratios/yield-analysis/${ticker.toUpperCase()}?apikey=${apiKey}&period=annual&fiscal_year_start=${fiveYearsAgo}&fiscal_year_end=${currentYear}&order=desc`
        );

        if (!annualResponse.ok) {
            throw createError({
                statusCode: annualResponse.status,
                statusMessage: `Error fetching annual yield analysis ratios from roic.ai`,
                message: annualResponse.statusText
            });
        }

        const quarterlyResponse = await fetch(
            `https://api.roic.ai/v2/fundamental/ratios/yield-analysis/${ticker.toUpperCase()}?apikey=${apiKey}&period=quarterly&fiscal_year_start=${fiveYearsAgo}&fiscal_year_end=${currentYear}&order=desc`
        );

        if (!quarterlyResponse.ok) {
            throw createError({
                statusCode: quarterlyResponse.status,
                statusMessage: `Error fetching quarterly yield analysis ratios from roic.ai`,
        ...
        if (existingRecord) {
            const { error: updateError } = await supabase
                .from('ticker_searches')
                .update({
                    yield_ratios_data: ratiosData,
                    yield_ratios_updated_at: new Date().toISOString()
                })
        ...
        console.error('Error fetching yield analysis ratios data from roic.ai:', error);
        throw createError({
            statusCode: 500,
